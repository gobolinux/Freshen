...
class FreshenUI(object):
    ...
    def parse_var(string, **variableDefinitions):
        """Parse the value of a shell variable out of a file (recipe)

        Perform variable substitutions on the value
        """
        from string import Template
        if not hasattr(self.parse_var, 'definitions'):
            self.parse_var.definitions = {
                'goboPrograms': getGoboVariable('goboPrograms'),
        ...
        if not variableDefinitions:
            variableDefinitions = {}
        variableDefinitions.update(self.parse_var.variableDefinitions)
                    ...
                    for line in file:
                        if line.startswith('post_install_message='):
                            postMessages.append((program,
                                self.parse_var(line, target='$goboPrograms/%s/%s' %
                                                (program, version))))
                            break
                    file.close()
                if i == self.limit:
