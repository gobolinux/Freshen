...
version = {'major': 3, 'minor': 0, 'revision': 8, 'prerelease': 'SVN'}
requiredVersions = {
    'Scripts': '2.9.4',
    'Compile': '1.11.2',
}
...
def installUpdate(upd):
    ...
    if upd.type == 'recipe':
        if os.system("Compile -G -U -D -b %s" % (upd.url)):
...
def _cacheDir():
    cacheDir = os.environ.get('HOME') + '/.cache/Freshen'
    if 'XDG_CACHE_HOME' in os.environ:
        cacheDir = os.environ['XDG_CACHE_HOME'] + '/Freshen'
    return cacheDir
