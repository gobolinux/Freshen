...
version = {'major': 3, 'minor': 0, 'revision': 10, 'prerelease': 'SVN'}
requiredVersions = {
    'Scripts': '2.10.0',
    'Compile': '1.13.0',
}
...
def installUpdate(upd, symlink='yes'):
    ...
    if upd.type == 'recipe':
        if os.system("Compile -l " + symlink + " -G -U -D -b %s" % (upd.url)):
        ...
        if os.system("Compile -l " + symlink + " -G -U -D -b %s" % (upd.program)):
            raise InstallationError(upd.program)
    else:
