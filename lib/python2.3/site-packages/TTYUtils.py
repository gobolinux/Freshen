...
class ProgressBar:
	...
	def draw(self):
		...
		if self.size2:
			...
			if self.alwaysRedraw or points != self.points or pc != self.percent or points2 != self.points2 or pc2 != self.percent2:
			...
			points = int(float(self.value)/self.size*self.barWidth)
			pc = int(100*float(self.value)/self.size)
			if self.alwaysRedraw or points != self.points or pc != self.percent:
