...
class Freshen(object):
	...
	def installedPrograms(self):
		if self._installedProgramsCache:
			...
			_installedProgramsCache = KeyInsensitiveDict((prog, [v+'-'+r for v in installed[prog] for r in installed[prog][v]]) for prog in installed)
			return self._installedProgramsCache
			
	
	def latestInstalled(self, program):
		if not self._currentVersionCache:
			_currentVersionCache = KeyInsensitiveDict()
		if program in self._currentVersionCache:
			...
			if program in self._installedProgramsCache:
				_currentVersionCache[program] = GuessLatest(_installedProgramsCache[program])
			else:
				_currentVersionCache[program] = None
			return self._currentVersionCache[program]
	...
	def updates(self):
		...
		toExamine,mode,skipSet = self.args.examineSet,self.args.mode,self.args.skipSet
		yieldCount = 0
		if self.args.limit: limit = int(self.args.limit)
		else: limit = None
		# Load from cache, and yield any programs in there up to the limit
		programList,considered,listComplete = loadCache()
		...
		for program,version,revision,type,url,intBy in programList:
			...
			if not skip(program, version, revision):
				yieldCount+=1
				pb.inc2()
		...
		for examinedProgram in toExamine:
			...
			if limit and pb.value2 >= limit:
				saveCache(programList, considered, False)
				raise StopIteration
			if skip(examinedProgram, None, None): continue
			...
			for program,version,revision,type,url in (upd for upd in orderedMatches if upd[0] not in considered):
				...
				if type != 'installed' and type is not None:
					...
					insVer = latestInstalled(program)
					if not insVer or (insVer != ver and GuessLatest((ver, insVer)) == ver):
						...
						if not skip(program,version,revision) and (not limit or yieldCount < limit):
							yieldCount+=1
							if limit and yieldCount>=limit: pb.clear()
							yield update
							pb.inc2() # Increment yielded update bar
		saveCache(programList, considered, True)
		pb.clear()
	...
	def skip(self, program,version,revision):
		...
		spl = program.split('-')
		if len(spl)>1 and spl[0] in args.skipSet: return True
		return False
	...
	def cacheFile(self):
		...
		return "%s/%s-%u"%(cacheDir, self.args.mode, self.args.cacheDataID)
	...
	def loadCache(self):
		...
		if self.args.forceCache or not self.args.noCache and os.path.exists(cacheFile) and os.path.exists(cacheFile) and time.time()-os.path.getmtime(cacheFile)<1800:
	...
	def fetchAvailables(self, forceNoCache=False):
		...
		elif not forceNoCache and not self.args.noCache and os.path.exists(cacheFile) and (self.args.forceCache or time.time()-os.path.getmtime(cacheFile)<1800):
			f = open(cacheFile, 'r')
