...
class Freshen(object):
	...
	def __init__(self, mode='updating', examineSet=None, skipSet=None, noCache=False, forceCache=False, types=None):
		if examineSet:
	...
	def updates(self, onExamine=None):
		...
		try:
			...
			for examinedProgram in toExamine:
				...
				if self.mode == 'unordered':
					packages = FindPackage(examinedProgram, types=self.types, availables=global_available_packages_cache, accessWeb=False)
					newest = packages[0]
					if 'recipe' == newest[3]:
						enabledFlags = UseFlags(newest[4])
						# Calculate the disabled flags from the potentials and enableds
						disabledFlags = potentialFlags(newest[4]) - enabledFlags
						newest = (newest[0], newest[1], newest[2], newest[3], newest[4], newest[0], enabledFlags, disabledFlags)
					else:
						newest = (newest[0], newest[1], newest[2], newest[3], newest[4], newest[0], frozenset(), frozenset())
					programList.append(newest)
					yield newest
					continue
					
				# Process actual results from here
				orderedRules,orderedMatches = CheckDependencies(examinedProgram, acceptable_types=self.types, mode=mode, recursive=True)
				thisProgramUpdates = []
	...
	def installUpdate(self, update):
		...
		if type == 'recipe':
			...
			print "InstallPackage -s keep -k -U -s keep -b -D --unmanaged=install %s"%(url)
			if os.system("InstallPackage -s keep -k -U -s keep -b -D --unmanaged=install %s"%(url)):
				raise InstallationError(program)
		else:
	...
	def fetchAvailables(self, forceNoCache=False):
		...
		if global_available_packages_cache: return
			...
			try:
				...
				exc = sys.exc_info()
				raise CacheWarning("Unable to save program data cache. ~/.Settings/freshen_cache or its\ncontents may be non-writable (%s.%s: %s)"%(exc[0].__module__, exc[0].__name__, exc[1]))
			finally:
				if f: f.close()
