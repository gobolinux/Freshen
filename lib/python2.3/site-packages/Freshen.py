...
from FreshenPythonUtils import *
import pickle
...
class Freshen(object):
	...
	def installedPrograms(self):
		...
		if not self.availables: self.fetchAvailables()
		installed = self.availables['installed']['programs']
	...
	def latestInstalled(self, program):
		...
		if program in self.installedPrograms():
			return GuessLatest(self.installedPrograms()[program])
		return None
	...
	def latestAvailable(self, program):
		...
		for type in self.types:
			typedata = self.availables[type]['programs']
	...
	def updateAvailable(self, program):
		...
		if self.noRevisions:
			latestInstalled,_ = Split_Version_Revision(latestInstalled)
			latestAvailable,_ = Split_Version_Revision(latestAvailable)
		return (
			GuessLatest((latestInstalled, latestAvailable))
			!= latestInstalled)
		
	
	def _unordered_update(self, examinedProgram):
	...
	def _make_update(self,upd,examinedProgram):
		if 'recipe' == upd.type:
	...
	def _program_updates_each(self, upd, examinedProgram):
		...
		if upd.type != 'installed' and upd.type is not None:
			...
			insVer = self.latestInstalled(upd.program)
			insVers = insVer and self.installedPrograms()[upd.program] or ()
			...
			if not insVer or ver not in insVers:
				...
				if self.noRevisions and insVer:
					iv,_ = Split_Version_Revision(insVer)
					if version == iv:
						return None
				if not self.skip(upd):
					return self._make_update(upd, examinedProgram)
		return None
	
	def _program_updates(self, examinedProgram, considered, programList):
		...
		orderedRules,orderedMatches = CheckDependencies(
			...
			mode=mode, recursive=True, availables=self.availables)
		thisProgramUpdates = []
		frz = frozenset()
		...
		for upd in (Update(upd, activeFlags=rule.get('activeflags', frz))
			...
			update = self._program_updates_each(upd, examinedProgram)
	...
	def _updates_from_cache(self, programList, listComplete):
		if listComplete:
			self.updateCount = len(programList)
		for update in programList:
			# Check if this should be included, and yield it if so.
			if not self.skip(update):
				yield update
	
	def _updates(self, toExamine, mode, progList, considered, onExamine):
		for program in toExamine:
			...
			if self.skip(program, None, None): continue
			if mode == 'unordered':
				newest = self._unordered_update(program)
				progList.append(newest)
				yield newest
				continue
			# Process actual results from here
			for upd in self._program_updates(program, considered, progList):
				yield upd
	...
	def updates(self, onExamine=None, last=False, saveCache=True):
		...
		for upd in self._updates_from_cache(programList, listComplete):
			yield upd
		...
		try:
			...
			# to examine.
			for u in self._updates(toExamine, mode, programList, considered,
			    onExamine):
				yield u
			listComplete = True
		finally:
	...
	@memoised
	def skip(self, upd, version=None, revision=None):
