...
class Freshen(object):
	...
	def installUpdate(self, update):
		...
		if type == 'recipe':
			if os.system("Compile -G -U -D -b %s"%(url)):
				self.callback('error', "Compilation of %s failed. Terminating update process. You should review the previous lines to see where the error arose."%(program))
				return True
		elif type == 'official_package':
			if os.system("InstallPackage -k -U -s keep -b -D -u keep %s"%(url)):
				self.callback('error', "Installation of %s failed. Terminating update process. You should review the previous lines to see where the error arose."%(program))
				return True
		else:
			self.callback('error', "Unknown action '%s'!"%(type))
			return True
		return False
	...
	def saveCache(self, programList, considered, listComplete):
		...
		try:
			...
			self.callback('error', 'Could not save cache data. Ensure that ~/.Settings/freshen_cache and its contents are writable.')
		finally:
			f.close()
